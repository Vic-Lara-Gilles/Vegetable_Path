...
**DON'T**: Use `--no-verify` — pre-commit hooks (lint, tests) MUST always run
...
**Skipping hooks** - Using `--no-verify` to bypass lint/test checks  
...
- Native git hooks (`.githooks/`) - Pre-commit validation via `core.hooksPath`
